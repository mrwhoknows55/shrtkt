...
let randomNum = getRandomNumber(1, 7500);


export let options = {
    stages: [
        { duration: '10s', target: vus }, // ramp-up to 20 VUs over 30 seconds
...
export default function () {
    let response = http.get(`http://localhost:${port}/expand/${randomNum}`)
    check(response, {
